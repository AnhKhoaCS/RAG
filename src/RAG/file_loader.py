...
class PyPDFLoader(BaseLoader):
    ...
    def __call__(self, pdf_file: List[str], **kwargs):
        num_processes = min(self.num_process, kwargs["workers"])
        with multiprocessing.Pool(process=num_process) as pool:
            doc_loader = []
...
class Loader:
    ...
    def load(self, pdf_file: Union[str, List[str]], workers: int = 1):
        if isinstance(pdf_file, str):
            pdf_file = [pdf_file]
        doc_loader = self.doc_loader(pdf_file, workers = workers)
        doc_split = self.doc_splitter(doc_loader)
        return doc_split
    def load_dir(self, dir_path: str, worker: int = 1):
        if self.file_type == "pdf":
            file = glob.glob(f"{dir_path}/*.pdf")
